# Your proxy never frees the real `dxgi.dll` — a game that unloads and reloads it walks straight past you
...
(`LoadLibraryA(sysdir)`, around line 84) and **contains no `FreeLibrary` call anywhere**
...
options change — the system copy stays resident under the base name `dxgi.dll`. The game's next
`LoadLibrary("dxgi.dll")` matches **it** by name and succeeds at once. The application directory is never
...
Wake, and note this proxy is the one currently carrying live camera work, so a silent bypass would be read as a probe regression. It is worth closing anyway, because when it does bite it costs a whole session to
